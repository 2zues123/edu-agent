"""Code Learning AI Agent — text Q&A via DeepSeek, image recognition via OpenAI Vision."""
...
class CodeLearningAgent:
    """Agent for code learning Q&A with optional image recognition."""
    ...
    def __init__(self) -> None:
        ...
        # Vision model (OpenAI)
        self.vision_api_key = os.getenv("OPENAI_API_KEY") or ""
        self.vision_base_url = os.getenv("OPENAI_BASE_URL") or "https://api.openai.com/v1"
        self.vision_model = os.getenv("CODE_VISION_MODEL") or "gpt-4o"
    ...
    def vision_client(self) -> OpenAI:
        if self._vision_client is None:
            if not self.vision_api_key:
                raise RuntimeError("Missing OPENAI_API_KEY in .env — required for image recognition")
    ...
    def answer_text_question(
        ...
        *,
        ...
        Returns:
            CodeAgentAnswer with the AI response.
        """
        user_prompt = (
            build_code_text_prompt(question, code_context)
            + "\n\n请额外给出「关联课程」和「下一步练习」两个小节，把代码问题对应到软件工程专业课程或能力点。"
        )
    ...
    def recognize_code_image(self, image_bytes: bytes, mime_type: str) -> str:
        """Recognize code from an image screenshot using OpenAI Vision.
        ...
        Returns:
            Recognized code text from the image.
        ...
        if not self.has_vision:
            raise RuntimeError("需要配置视觉模型 API Key（OPENAI_API_KEY）才能使用图片识别功能。")
    ...
    def answer_with_image(
        ...
        """Recognize code from an image, then answer the user's question.

        This is a two-step process:
        1. Call OpenAI Vision to recognize code from the image.
        2. Call the text model (DeepSeek) to answer based on the recognized code.
        ...
        Returns:
            CodeAgentAnswer with recognized code and AI response.
        """
        # Step 1: Recognize code from image
        recognized = self.recognize_code_image(image_bytes, mime_type)

        # Step 2: Answer question using text model
        text_prompt = (
            build_code_image_answer_prompt(question, recognized)
            + "\n\n请额外给出「关联课程」和「下一步练习」两个小节，把截图中的代码对应到软件工程专业课程或能力点。"
        )
        response = self.text_client.chat.completions.create(
            model=self.text_model,
            messages=[
        ...
        # Try to detect language from recognized code
